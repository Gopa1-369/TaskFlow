...
const Layout = ({ onLogout, user }) => {
  ...
  //all the ways task is completed or not
  const stats = useMemo(() => {
    ...
    const totoalCount = tasks.length;
    const pendingCount = totoalCount - completedTasks;
    const completionPercentage = totoalCount
      ? Math.round((completedTasks / totoalCount) * 100)
      : 0;
    ...
    return (
      <div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-100 hover:shadow-md transition-all">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-purple-100">{icon}</div>
          <div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="ml-0 xl:ml-64 lg:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
          ...
          {/*not token found ,have not log in */}
              ...
              <div className="space-y-2 sm:space-y-3">
                <div className="flex items-center justify-between text-gray-700">
                  ...
                  <span className="text-xs bg-purple-100 text-purple-700 px-1.5 py-0.5 sm:px-2 rounded-full">
                    {stats.completedTasks}/{stats.totoalCount}
                  </span>
                </div>
                ...
                {tasks.slice(0, 3).map((task) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-grau-500 mt-0.5">
                        {task.createdAt
                          ? new Date(
                              task.createdAt
                            ).toLocaleDateString()
