      ...
      <div className={CT_CLASSES.header}>
        ...
        <div className={CT_CLASSES.sortContainer}>
          <div className={CT_CLASSES.sortBox}>
            ...
            {/* MOBILE DROPDOWN */}
            <select value={sortBy} onChange={e =>sortBy(e.target.value)} className={CT_CLASSES.select}>
      ...
      <div className={CT_CLASSES.list}>
        {sortedCompletedTasks.length === 0 ? (
          ...
          sortedCompletedTasks.map(task=> (
            <TaskItem key={task._id || task.id} task={task} onRefresh={refreshTasks} showCompleteCheckbox={false} className='opacity-90 hover:opacity-100 transition-opacity md:text-base'/>
          ))
        )}
